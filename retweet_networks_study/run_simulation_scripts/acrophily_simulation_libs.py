...
# Print checkpoint for grid:
print('dataframes read', flush=True)

# Simulating retweet network of left-leaning individuals

print('beginning liberals simulation', flush=True)
orient = 'left'
min_tweets = 5
...
for thresh in range(range_start, range_end, 5):
    models = ['homo_right', 'homo', 'empi', 'rand']
    for b in models:
        print('threshold = {0}, model = {1}'.format(thresh, b), flush=True)
        if b == 'homo':
            ego, peer, peer_higher, peer_higher_ci = repeat_base_rating(r, u, thresh=thresh, baseline='homo', n=n)
        if b == 'homo_right':
            ego, peer, peer_higher, peer_higher_ci = repeat_base_rating(r, u, thresh=thresh, baseline='homo_right', n=n)
        else:
...
print('results saved as pickle file', flush=True)

# baseline_left = pickle.load( open( "./data/tests/baseline_left_5_40_.pickle", "rb" ) )
...
pd.DataFrame(list(zip(x, pphb, pphb_ci, pphbh, pphbh_ci, pphba, pphba_ci, pph, pph_ci)),
             columns=['threshold',
                      ...
                      'homophily_right_ci',
                      'empirical',
                      'empirical_ci']).to_csv('../data/left_acroph_sim_'+str(start_range)+'_'+str(end_range)+'.csv', index=False)

print('dataframes saved', flush=True)
