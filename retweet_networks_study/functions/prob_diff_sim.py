...
def get_sim_df(users_df, rt_df, orient, n=100):
    ...
    print('Data preprocessed. Joining data.', flush=True)
    ...
    for i in range(n):
        ...
        more_extreme_count_df = more_extreme_count_df.append(rt_df_subset, ignore_index=True)
...
def save_data(df, orient, frac_start, frac_end):
...
def run_sim(orient, frac_data=False, frac_start=0.0, frac_end=0.1):
    ...
    print('Data loaded successfully. Preprocessing data.', flush=True)
    # Preprocess data:
    rt_df_frac = preprocess_data(users_df, rt_df, orient, frac_data=frac_data, frac_start=frac_start, frac_end=frac_end)

    # Run simulation/save dataframe:
    prob_diff_df = get_sim_df(users_df, rt_df_frac, orient)

    # Save dataframe:
    save_data(prob_diff_df, orient, frac_start, frac_end)
