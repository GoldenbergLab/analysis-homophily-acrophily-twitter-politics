...
df <- read.csv('../data/mean_abs_diff_right_long.csv')

df_alt <- df[df$threshold %in% seq(5, 40, 5),]
```
...
mean_abs_diff_plot <- df %>%
  ggplot(aes(x=threshold, y=mean_abs_diff, group=abs_diff_condition)) +
  geom_line()+
  geom_ribbon(aes(ymin=ci_lower, ymax=ci_upper, fill=abs_diff_condition), alpha=0.3)+
...
acroph_plot <- acroph_df_long %>%
  ...
  scale_fill_manual(labels = c("Acrophily", "Empirical", "Homophily"), values = c("red", "orange", "blue"))+
