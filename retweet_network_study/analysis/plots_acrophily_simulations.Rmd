...
d_rt %>% 
  ...
  ggplot(aes(x=orig_rating_ego, vjust=-0.2, color=poli_affil)) +
  geom_histogram(fill='transparent') +
  facet_wrap(~poli_affil) +
  labs(x='User Political Rating', y='Count',
       title='User Political Rating by Political Affiliation', color = '') +
  scale_color_manual(labels = c("Conservatives", "Liberals"), values = c("red", "blue"))+
  theme_minimal() +
