...
df_long <- df_long %>% 
  rename(avg_perceived_copartisan = avg_libcons) %>%
...
## Co-partisan model centering left-wing participants
...
# Centering intercept of non-moderate groups:
df_co$is_moderate <- relevel(factor(df_co$is_moderate), 'no')

# Full model with all interactions:
co_mod_left <- lm(avg_sentiment ~avg_perceived_copartisan*poli_affil*is_moderate, data=df_co)
...
## Co-partisan model centering right-wing participants

```{r}

df_co$poli_affil<- relevel(factor(df_co$poli_affil), 'right')
co_mod_right <- lm(avg_sentiment ~ avg_perceived_copartisan*poli_affil*is_moderate, data=df_co)
summary(co_mod_right)

```

## Cross-partisan model centering left-wing participants

```{r}

df_cross$poli_affil <- relevel(factor(df_cross$poli_affil), 'left')
df_cross$is_moderate <- relevel(factor(df_cross$is_moderate), 'no')

cross_mod_left <- lm(avg_sentiment ~ avg_perceived_copartisan*poli_affil*is_moderate, data=df_cross)
summary(cross_mod_left)

```

## Cross-partisan model centering riht-wing participants

```{r}
df_cross$poli_affi <- relevel(factor(df_cross$poli_affil), 'right')

cross_mod_con <- lm(avg_sentiment ~ avg_perceived_copartisan*poli_affil*is_moderate, data=df_cross)
summary(cross_mod_con)
```
