...
# Function to run homophily simulation and append results to dataframe:
...
def get_sim_df(r, n=100):
    ...
    for i in range(n):
        ...
        print(more_extreme_count_df.shape)
        print(more_extreme_count_df.columns.values)

    # Gets the probability that a peer is more extreme in each condition for each ego across all trials:
...
# Subsetting fraction of users to speed up simulation:
frac_start = 0.9
frac_end = 1.0
...
file_path = '../data/prob_diff_libs_'+str(frac_start)+'_'+str(frac_end)+'.csv'
